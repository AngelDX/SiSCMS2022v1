                      ...
                      <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-8 shadow-lg rounded-lg">
                        <div class="px-4 py-5 flex-auto">
                          <div class="mb-2">
                                ...
                                <img src="@if($post->image){{Storage::url($post->image->url)}}@else /storage/posts/default.png @endif" >
                          </div>
                          @foreach ($post->tags as $tag )
                              <a href="{{ route('posts.tag',$tag) }}" class="inline-block px-3 h-6 bg-purple-600 text-white rounded-full text-sm shadow-md">{{$tag->name}}</a>
                          @endforeach
